...
- 附：负责人要求「看看卡片长什么样」，本轮用**真实标记 + 真实样式表**在浏览器里渲染了一张示例卡（数据是示例：工学 ‹计算机类 / 电子信息类 / 机械类›、理学 ‹数学类 / 物理学类›，卡片结构、文案、按钮与线上完全同源），只存在于浏览器 DOM 里，刷新即消失，仓库里没有任何预览文件。浏览器抓图这一侧连续报 `capture failed for guest`，因此没有留下截图文件。
