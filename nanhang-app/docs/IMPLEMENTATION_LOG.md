...
- 本轮实测（2026-09-14）：
  ...
  - `on`：参数已全部补齐（`ZoneId=160001` 即 ap-chengdu-1 的数字 ID；TypeId 17 / 256MB / 1 副本经 `DescribeProductInfo` 确认按量可售；VPC/子网沿用原值），创建请求被腾讯云以 `ERR_INSUFFICIENT_BALANCE`（余额 -13、冻结 4）拒绝——**欠费账户不能创建任何按量资源**。这一步不产生费用，也说明开启功能只差充值。
- 未做与边界：没有做网页的视觉/交互验收（按项目规则由负责人自行验收）；`on` 里「创建成功之后」那段（外网地址开通 → 回填环境变量 → 站点转 Redis）因欠费无法实测，充值后第一次开启如果报错，把助手窗口里的步骤发回来即可定位；本机若要连 Redis 自检，密码只在云函数环境变量里，不在本机留副本。
