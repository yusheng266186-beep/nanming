...
- 目的：负责人要求继续更新前端，且后端主动匹配，不能擅自修改前端。本轮读取并收集已交还的当前工作区增量，未编写或回滚 `apps/web` 源码。
...
- 验证：`npm run typecheck` 通过；`npm test -- --maxWorkers=2` 为 43 文件 532 项通过、0 失败；正式 API/COS 地址下 `npm run web:build` 通过，主 JS 339.89 kB（gzip 110.36 kB）。`node scripts/build_function.mjs` 打包成功。
