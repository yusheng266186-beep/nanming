...
- 基线 `7cd9fc6`；主入口核对为 `main.tsx → App` 六章。通过 GitHub CLI 读取北辰 `index.html`（`682aff411e492946b894e416fd445c5c0f85e053`），参考窄面板、行内状态和分段选择；沿用南溟纸色、海绿、铜线与宋体。
- 起航：固定画框内缘，用慢速潮汐与光感适配触屏；指针视差改写局部 CSS 变量，不再每个 pointermove 重渲染应用；取消拖页也触发的点水涟漪。行装清单桌面三项并列，手机顺序填写、触控尺寸保留；选填目标分不再抵扣必填缺项，解决选科没齐却显示「还差 0 件」。
- 抽屉：起航、分数轴、航线图共用 480ms 收尾曲线；院校正文首次展开才挂载，关闭时保留 DOM，快速反向也连续；收起设置 `aria-hidden`/`inert`。`overflow:clip` 保持 sticky；翻页落点改读每张卡解析后的 top，减小滑动滞后，关闭的卡堆跳过计算。
- 新增 `motion.ts` 同时订阅系统偏好和会话开关，首页/卡堆/谈心即时响应；关闭动效归零卡片姿态。定位细化读数分层与窄屏布局，输入焦点反馈不改变行内边距。设置收为 540px 面板，用分隔线、行内开关、三档选择与共享说明，长发布版本允许折行。
- 验证：web typecheck 通过；全量 `npm test -- --run` **54 文件 / 592 项通过**；生产构建通过（72 modules）；新增可重跑 `scripts/check-ui-polish.mjs`，纯合成夹具 **23 项 DOM 断言通过**，不截图。脱敏汇总见 [本轮验证](verification/ui-polish-2026-09-20.json)，细项见 [DOM 结果](verification/ui-polish-dom-2026-09-20.json)。首次全量失败的两条测试按「最后一个媒体查询」取规则，已改为定位对应规则；历史中间结果 585 项不代表最终版本。
- 机器本次实测 Node 26.7.0 / npm 11.19.0，浏览器为独立无账号 Edge headless；未连接真实学生、学校、AI 服务，未做视觉或真机验收。TASK-13/14 及云端开关状态未变。
- 交付隔离：完整工作区保留已有数据管线、数据库和截图修改，独立 `codex/frontend-polish` 分支只带本轮前端、测试、文档及生成文本；既有数据发布状态仅作为主进度的历史事实沿用，不在本轮重新发布数据。同步工具补 `.git` 文件排除，避免 linked worktree 元数据进入索引/哈希。
