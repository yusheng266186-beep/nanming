...
当前前端会在谈心素材满足门槛后，再调用一次原有 `POST /v1/career/turn`，`user_text` 固定以「【收尾】谈心到这里。」开头，携带学生原话证据与已发布专业目录，要求最终小结、不追问、无可点选项。该调用没有新增字段或数据库操作。旧千帆提示的每轮追问与 guided 选项会覆盖此意图；API 版本 10 对这一固定前缀切换收尾规则，`finalize()` 强制清空收尾选项，普通轮不变。服务端仍按原证据 ID 与目录 ID 校验建议；前端现有单轮最多取 4 条，最终建议与之前建议合并，方向页总量按前端既有规则控制。合成数据线上收尾轮 HTTP 200、2 条有据建议、空选项、无追问；见 `verification/final-turn-online-2026-09-13.json`。本轮没有改前端源码、学校数据库或密文对象；Pages `c0b2664` 已部署并通过公网资产校验。

以 `apps/web/src/main.tsx` 实际挂载的 `App.tsx` 为准；JourneyApp 为参考壳。保留其他任务的界面与规则，视觉验收由负责人完成。后端对齐与881个学校密文已经上线；TOTP 自 API 版本9起启用，当前 API 版本10另覆盖自动收尾轮。
...
| 引航/泛舟、三档思考、原话与库内建议 | `/v1/access/exchange`、`/v1/career/turn` → 会话存储（配共享 Redis 时为 `RedisStateStore`）→ 千帆 | 单条原话容量从 500 对齐到 4000 字，12 条合计上限 48000；8 条上下文总上限 32000；HTTP 体上限 512 KiB，仍严格拒绝客户端控制模型及库外建议。2026-09-14 起云端 Redis 已冻结，实际走单实例内存档 |
| AI 谈心动态码 | `/v1/access/exchange` → TOTP 校验 → 状态存储一次性消费 → 会话 | 正式环境由固定访问串改为 SHA-1/30 秒/6 位 TOTP，允许前后一个窗口；同一码只成功一次，旧固定码已撤销。共享 Redis 冻结后，一次性消费只在单实例进程内生效，重启即失效 |
