...
- 源工作簿制作方与全部字段准确性**未经独立核实**，`user_provided` / `NOT_VERIFIED`。库不是发布数据，不得当作正式推荐依据。**例外**：一分一段表来自四川省教育考试院官方发布，已按官方内容逐行核对（见 [一分一段表](TASK03_SCORE_DISTRIBUTION.md)）。
- 学费币种与计费周期未核实，`tuition_comparable = 0`；中外合作与港澳台院校可能以外币标示。
- 历史记录的科类、批次、录取类型与跨年可比性未知，`comparability = NOT_ESTABLISHED`。专业组逐年重组，跨年可比性需人工核实。
- `计划类别` 空值按普通类处理，属推断值。
...
本库不实现、也不声称实现：正式发布流程、跨年 `comparability_link` 人工核实、加分情景、2025/2024/2023 分段表定位、概率或录取预测。
