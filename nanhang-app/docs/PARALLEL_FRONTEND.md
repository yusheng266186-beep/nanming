...
> 下一步：依次发布学校汇总密文、新版API与Pages，再做线上完整流程、学生规模与回滚验收；TASK-11保持跳过。完整进度及操作见[项目进度](PROJECT_STATUS.md)。历史验证记录不代表当前状态。
<!-- PROJECT-STATUS:END -->
...
四班查询码轮认领：`quality-huixi.ts` 的查询码规范化、`chapters/locate.tsx` 的查询码输入、`App.tsx` 的核验提示；仅兼容身份证末位 X 与规则说明，不改布局/样式。状态：进行中。
