...
实现状态（2026-09-10）：已按该固定提交取得并核对 `types.ts`、`parser.ts`、`score-validation.ts` 及必要依赖，在 `nanhang-app/packages/school-adapter` 实现 `adaptAccuracyV12SelectedStudent` 单人入口、真实字段结构合成样例和冲突回归。完整映射、Git blob 和未知值处理见 [accuracy-v1.2 单人摘要字段映射](../ACCURACY_V1_2_FIELD_MAPPING.md)。没有真实工作簿，本状态不代表实数据验收。
