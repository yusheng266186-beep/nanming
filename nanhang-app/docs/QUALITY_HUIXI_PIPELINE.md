...
## 2026-09-13 前端数据来源静态审阅

[前端专项](FRONTEND_CODE_REVIEW_2026-09-13.md) F03/F05 指出学校和手填复用 form.exams，以及无可用切线时裸分回退为高考等价分的问题。学校源分片本身未变，但前端派生数据和展示来源可能不一致；待后续修复隔离。本轮没有访问真实分片、启动页面或执行学校查询。


## 2026-09-13 审阅补充（当前安全边界）

学校 API 的限流代码支持 Redis 共享计数，云端 Redis 于 2026-09-14 按计费冻结销毁后实际走单实例内存计数（重启清零），仍需验收故障策略和校园出口。开发 Vite 仍在 `apps/web/vite.config.ts` 挂载 `/data/quality-huixi`，可绕过姓名与查询码直接访问产物；“不再由 Vite 暴露”不能作为当前结论。范围限开发服务，未据此认定 Pages 泄露。本轮未读取真实分片作攻击验证，处理建议见[全项目审阅](PROJECT_REVIEW_2026-09-13.md)。
...
2. **服务端校验 + 共享限流。** 学生只经 `POST /v1/school/identify`（姓名 + 验证码），
   服务端查身份索引后只返回本人分片；限流用共享计数（同一姓名 5 次 / 同一 IP 120 次，15 分钟窗），
   配了共享 Redis 时多实例下也生效——这是「离线穷举」在接口这一侧的成本来源。2026-09-14 起云端
   Redis 已按计费冻结销毁，实际为单实例进程内计数，重启清零；恢复共享计数后此条才重新成立。
