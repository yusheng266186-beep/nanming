...
export function buildQianfanSystemPrompt(request: UpstreamRequest): string {
  ...
  return [
    ...
    "",
    // 语言要求放在最前面、单独一段：学生能实时看到思考过程（见文件顶部第 1 条），
    // 而只写在【输出格式】里时，线上实测思考仍会整段用英文（2026-09-20 截图证据）。
    "【语言】全部输出、包括你回答前的思考过程，都必须用简体中文。不要用英文推理："
      + "学生会在等待时实时读到你的思考，英文他读不懂。",
    "",
    ...(finalTurn ? [
      "【收尾轮】",
    ...
    "- 然后只问一个问题。问题要能让学生讲出一段经历、一个感觉或一次取舍，"
      ...
      + "整段回复里只留一个问号：接住他的话时不要顺手再问一句，那会变成两个问题。",
