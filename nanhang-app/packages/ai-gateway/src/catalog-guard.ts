...
export function parseDirectionCatalog(
  raw: unknown,
): readonly CatalogChoice[] | null {
  ...
  for (const value of raw) {
    ...
    if (Object.keys(item).some((key) => key !== "id" && key !== "name"))
      return null;
    if (
      ...
      return null;
    if (
      item.id !== `catalog:${item.name.normalize("NFKC").trim()}` ||
      seen.has(item.id)
    )
      return null;
